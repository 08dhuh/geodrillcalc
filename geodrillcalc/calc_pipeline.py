...
class CalcPipeline:
    ...
    This class requires a fully instantiated and initialised WellBoreDict instance, achieved through the WellBoreDict.initialise method.
    The WellBoreDict instance attributes are updated in three consecutive calls to the class methods.
    
    The CalcPipeline class is intended to be used in conjunction with the WellBoreDict instance, collaborating to calculate wellbore parameters.
    ...
    For the casing stage, an additional argument is required, distinguishing between production and injection pumps 
    ...
    Usage Example:
    wbd = WellBoreDict()
    wbd.initialise_and_validate_input_data(depth_data=depth_data, **initial_values)  # Refer to WellBoreDict.__doc__ for the required keyword arguments.
    ...
    def calc_pipeline(self, is_production_pump):
        try:
            ...
            self.logger.error(f"An error occurred during calculation: {str(e)}")
    ...
    def _interval_pipeline(self):
        ...
        interval_df = pd.DataFrame(self.casing_diameters_in_metres,
                                   columns=['prod_casing_diameters']) #storing the calculated results in a dataframe
        ...
        interval_df['production_screen_diameters'] = interval_df.apply(lambda row: find_next_largest_value(row['production_minimum_screen_diameters'],
                                                                                                           self.casing_diameters_in_metres) if not np.isnan(row['production_minimum_screen_diameters']) else np.nan,
                                                                       axis=1)
        ...
        min_total_casing_production_screen_diameter = interval_df.iloc[interval_df['total_casing'].argmin(
        ...
        ir['production_screen_diameter'] = max(min_total_casing_production_screen_diameter, self.casing_diameters_in_metres[0]) 
        ir['injection_screen_diameter'] = wbd.drilling_diameter_data.loc[wbd.drilling_diameter_data['metres']==ir['injection_open_hole_diameter']]['recommended_screen'].iloc[0]
    ...
    def _casing_pipeline(self, is_production_pump):
        ...
        casing_stage_data = wbd.casing_stage_data.copy().drop(['drill_bit'], axis=1)
        #print(casing_stage_data)
        screen_diameter =  wbd.production_screen_diameter if is_production_pump else wbd.injection_screen_diameter
        screen_length = wbd.production_screen_length if is_production_pump else wbd.injection_screen_length
        #------pre-collar
        pre_collar = [
        ...
        logger.info(f'superficial_casing_required: {superficial_casing_required}')
        ...
        logger.info(f'separate_pump_chamber_required: {separate_pump_chamber_required}')
        ...
        casing_stage_data.loc['production_screen'] = [*cc.calculate_screen_depths(wbd.depth_to_top_screen,
                                                                                  ...
                                                                                  wbd.aquifer_thickness),
                                          screen_diameter]
        casing_stage_data['drill_bit'] = casing_stage_data.apply( lambda row: cc.calculate_drill_bit_diameter(row['casing'], 
